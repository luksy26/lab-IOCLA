...
void reversePic(Picture *pic) {
	...
	Picture *reversed = generatePicture(pic->height,pic-> width,pic-> pix_array);
	for(int i = 0; i < pic->height; i++){
		for(int j = 0; j < pic->width; j++){
			reversed->pix_array[i][j] = pic->pix_array[pic->height - 1 - i][j];
		}
	}
	
	
	printPicture(reversed);
	freePixelArray(&reversed->pix_array, reversed->height, reversed->width);
	freePicture(&reversed);
